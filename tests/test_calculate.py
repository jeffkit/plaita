...
class CalculateTestCase(TestCase):
    ...
    def test_add(self):
        flow = Flow(
            ...
            version="1",
            runtime="python",
            input_type=Property(data_type=types.INTEGER, is_required=True),
            output_type=Property(data_type=types.INTEGER),
            nodes=[
                Start(id="start", next="cal"),
                Calculate(
                    id="cal",
                    expression={"function_name": "add", "params": {"left": "$INPUT", "right": 1}},
                    next="end",
                ),
                End(id="end", **{"resultType": "success", "output": "$NODE.cal"}),
            ],
        )
        self.assertEqual(2, flow.run(1))
    ...
    def test_greeting(self):
        flow = Flow(
            ...
            nodes=[
                ...
                Calculate(
                    id="cal",
                    expression={"function_name": "concat", "params": {"left": "hello ", "right": "$INPUT"}},
                    next="end",
                ),
                End(id="end", **{"resultType": "success", "output": "$NODE.cal"}),
            ],
        )
        self.assertEqual("hello KongJie", flow.run("KongJie"))
    ...
    def test_embed(self):
            ...
            nodes=[
                ...
                Calculate(
                    ...
                    expression={
                        ...
                        "params": {
                            "left": "$INPUT",
                            "right": {"function_name": "multiply", "params": {"left": "$INPUT", "right": 10}},
                        },
                    },
                    next="end",
                ),
        ...
        self.assertEqual(11, flow.run(1))
