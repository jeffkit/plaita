...
节点（`Node`）是流程的最小执行单元。plaita **默认注册表含 23 种**内置节点（另有 `CodeNode` 需 `register_code_node()` 显式注册），覆盖起止、赋值、分支、循环、并行、子流程、HTTP 与事件等场景；不够用时还可[自定义节点](custom.md)，或用 [plaita-ai 工具层](../ai/tools.md) 把数据源暴露给 Agent。
