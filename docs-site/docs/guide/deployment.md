...
| RBAC / 审计 / 凭据 | ✅ | ✅ | ✅ | — |
| 集群管理 / 队列 / 事件 | ❌ | ✅ | ✅ | — |
| 定时触发器（cron） | ✅ 内置调度循环 | ✅ schedule_service | ✅ | — |
| 执行日志查看 | ✅ SQLite | ✅ Redis Stream | ✅ | — |
| 挂起-恢复（审批/事件） | ✅ 单进程 | ✅ | ✅ | ✅（配 EventBus） |
| 适用场景 | 个人试用 / 演示 / 轻量自动化 | 团队共享编排台 | 生产规模 / 高可用 | 把编排嵌进你的应用 |

\* 本地单机档的挂起-恢复运行在 **console 单进程内**：checkpoint 存 SQLite，
重启后仍可显式恢复；但进程内事件订阅会随重启丢失，且 resume 必须走同一个
console 进程。多实例/跨进程的审批恢复请升到 Redis 档。
...
- 定时触发器由 console 内置调度循环承担（cron 语义与集群档一致），但无
  队列持久化：console 进程退出时进行中的本地执行会丢失
