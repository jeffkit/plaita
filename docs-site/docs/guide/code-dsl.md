# 类代码 DSL：S-expr 与 @flow

除了 [JSON / YAML / Python Builder](yaml-and-dsl.md)，plaita 还提供两种**更接近真实代码**的 flow 编写方式：

- **S-表达式 DSL**（`plaita.dsl.sexpr`）：Lisp 风格的括号语法，树形结构即 flow 结构，可编译、可校验、可逆。
- **AST Python DSL**（`plaita.dsl.codeflow`）：写一个普通 Python 函数，`@flow` 装饰器用 `ast` 把函数体编译成 flow。表达式层用真 Python 语法（`INPUT.age >= 18`），不再是字符串。

两者产出的都是同一个 `Flow` IR，跑在现有运行时上——断点续执、并行、错误处理、可视化、可审计全部保留。设计目标：**AI 是主要生成者，但人能读、能审、能编译期拦错**。

```mermaid
flowchart LR
  JSON["JSON"] --> IR["Flow IR"]
  YAML["YAML"] --> IR
  Builder["Python Builder"] --> IR
  Sexpr["S-expr"] --> IR
  Codeflow["@flow (AST)"] --> IR
  IR --> Runtime["执行引擎"]
```

!!! tip "做 AI Agent？"

    这三种 DSL 都是 Agent 把"计划"物化为"可执行流程"的载体，其中 `@flow` + [`flow_from_source`](#flow_from_source) 最适合 LLM 运行期生成（编译期校验 + 无源文件依赖）。端到端的 Agent 编排模式见 [应用场景 - Agent 编排](../scenarios/agent-orchestration.md)。
...
- **`reduce`**：当前运行时对 `reduce` 子流程输出的求值存在既有缺陷（非 DSL 层问题），`codeflow` 暂不提供 reduce 的可运行示例；`reduce` 编译支持已就位，待运行时修复后即可启用。
