...
| `docs-site/docs/distributed/flow-worker.md` / `index.md` / `guide/execution-modes.md` / `docs/event-ARCHITECTURE.md` | `plaita/server/flow_worker.py`, `plaita/server/task_queue.py`, `plaita/server/execution_lease.py`, `plaita/server/event_filter.py`, `plaita/core/strategies.py` | 可靠性边界；Stream 队列；PERSIST_EVERY_N_STEPS；resume lease |
